...
# %%
# ==========================================
# 3. Generate full 128-byte config zone header
# ==========================================
...
# Load factory config as base template
with open(
    os.path.join(script_dir, "atecc608b_tcustom_factory_config.json"), "r"
) as f:
    factory_config = json.load(f)

# Parse factory raw_hex into byte array
factory_bytes = [int(b, 16) for b in factory_config["raw_hex"].split()]
assert len(factory_bytes) == 128, f"Expected 128 bytes, got {len(factory_bytes)}"

# Start with factory defaults
config_zone = list(factory_bytes)

# Zero out bytes 0-15 (read-only, device-specific — atcab_write_config_zone skips these)
for i in range(16):
    config_zone[i] = 0x00

# Apply our computed SlotConfig (bytes 20-51)
for i in range(32):
    config_zone[20 + i] = slot_config[i]

# Apply Persistent Latch / VolatileKeyPermission (byte 69)
config_zone[69] = 0x8A

# Apply ChipOptions (bytes 90-91): IO Protection optional, Key is Slot 6
config_zone[90] = 0x02
config_zone[91] = 0x60

# Apply our computed KeyConfig (bytes 96-127)
for i in range(32):
    config_zone[96 + i] = key_config[i]

# Write C header file
output_path = os.path.join(script_dir, "..", "main", "atecc608b_config_zone.h")
...
with open(output_path, "w") as f:
    ...
    row_comments = [
        "Bytes   0-15:  Read-only (zeroed, skipped by atcab_write_config_zone)",
        "Bytes  16-31:  I2C config, SlotConfig[0-5]",
        "Bytes  32-47:  SlotConfig[6-13]",
        "Bytes  48-63:  SlotConfig[14-15], Counter[0], Counter[1]",
        "Bytes  64-79:  Counter[1] cont., UseLock, VolatileKeyPermission, ...",
        "Bytes  80-95:  UserExtra, Selector, LockValue, LockConfig, SlotLocked, ChipOptions, X509",
        "Bytes  96-111: KeyConfig[0-7]",
        "Bytes 112-127: KeyConfig[8-15]",
    ]

    for row in range(8):
        start = row * 16
        end = start + 16
        hex_bytes = ", ".join(f"0x{b:02x}" for b in config_zone[start:end])
        comma = "," if row < 7 else " "
        f.write(f"    {hex_bytes}{comma} // {row_comments[row]}\n")
...
print(f"\nGenerated: {os.path.abspath(output_path)}")
print(f"Config zone ({len(config_zone)} bytes):")
print(" ".join(f"{b:02X}" for b in config_zone))
