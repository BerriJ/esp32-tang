...
class TangKeyStore {
public:
  // Signing key (private part encrypted at rest, public in NVS)
  ...
  // Check if an encrypted signing key exists in NVS
  bool has_signing_key() {
    ...
    size_t len = 0;
    err = nvs_get_blob(handle, "sig_enc", nullptr, &len);
    bool found = (err == ESP_OK && len == P256_PRIVATE_KEY_SIZE);
    nvs_close(handle);
    return found;
  }

  // Generate signing keypair, encrypt private key with AES-256-GCM, store all
  bool generate_and_encrypt_signing_key(const uint8_t *aes_key) {
    ...
    memcpy(encrypted, sig_priv, P256_PRIVATE_KEY_SIZE);

    if (!AESGCM::encrypt(encrypted, P256_PRIVATE_KEY_SIZE, aes_key, 32, iv,
                         sizeof(iv), nullptr, 0, tag)) {
      ESP_LOGE(TAG_STORAGE, "Failed to encrypt signing key");
      return false;
    }
    ...
    ok = ok && (nvs_set_blob(handle, "sig_enc", encrypted,
                             P256_PRIVATE_KEY_SIZE) == ESP_OK);
    ok = ok && (nvs_set_blob(handle, "sig_iv", iv, sizeof(iv)) == ESP_OK);
    ok = ok && (nvs_set_blob(handle, "sig_tag", tag, GCM_TAG_SIZE) == ESP_OK);
    ...
    if (ok) {
      sig_loaded = true;
      ESP_LOGI(TAG_STORAGE, "Signing key generated and encrypted in NVS");
    }
    return ok;
  }
  ...
  // Decrypt signing private key from NVS using AES-256-GCM.
  // GCM tag verification acts as implicit password check.
  bool decrypt_signing_key(const uint8_t *aes_key) {
    nvs_handle_t handle;
    esp_err_t err = nvs_open("tang-server", NVS_READONLY, &handle);
    if (err != ESP_OK)
      return false;
    ...
    if (nvs_get_blob(handle, "sig_enc", encrypted, &len) != ESP_OK) {
      nvs_close(handle);
      return false;
    }

    len = sizeof(iv);
    if (nvs_get_blob(handle, "sig_iv", iv, &len) != ESP_OK) {
      nvs_close(handle);
      return false;
    }

    len = GCM_TAG_SIZE;
    if (nvs_get_blob(handle, "sig_tag", tag, &len) != ESP_OK) {
      nvs_close(handle);
      return false;
    }

    len = P256_PUBLIC_KEY_SIZE;
    if (nvs_get_blob(handle, "sig_pub", stored_pub, &len) != ESP_OK) {
      nvs_close(handle);
      return false;
    }
    ...
    if (!AESGCM::decrypt(encrypted, P256_PRIVATE_KEY_SIZE, aes_key, 32, iv,
      ...
      ESP_LOGW(TAG_STORAGE,
               "Signing key GCM tag failed — wrong password or wrong device");
      return false;
    }

    memcpy(sig_priv, encrypted, P256_PRIVATE_KEY_SIZE);
    P256::compute_public_key(sig_priv, sig_pub);

    // Verify computed public key matches stored public key
    if (memcmp(sig_pub, stored_pub, P256_PUBLIC_KEY_SIZE) != 0) {
      ESP_LOGE(TAG_STORAGE, "Signing public key mismatch after decryption");
      memset(sig_priv, 0, P256_PRIVATE_KEY_SIZE);
      return false;
    }

    sig_loaded = true;
    ESP_LOGI(TAG_STORAGE, "Signing key decrypted and verified");
    return true;
  }
