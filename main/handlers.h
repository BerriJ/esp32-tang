...
void handleAdv()
{
  ...
  sig_key["kty"] = "EC";
  sig_key["crv"] = "P-521";
  sig_key["x"] = base64_url_encode(tang_sig_public_key, 66);
  sig_key["y"] = base64_url_encode(tang_sig_public_key + 66, 66);
  ...
  rec_key["kty"] = "EC";
  rec_key["crv"] = "P-521";
  rec_key["x"] = base64_url_encode(tang_exc_public_key, 66);
  rec_key["y"] = base64_url_encode(tang_exc_public_key + 66, 66);
  ...
  if (!sign_ecdsa_p521(hash, 64, tang_sig_private_key, signature))
  {
    server_http.send(500, "text/plain", "Signing failed");
    return;
  }
  String signature_b64 = base64_url_encode(signature, 132);
...
void handleRec()
{
  ...
  uint8_t client_pub_key[132];
  int x_len = base64_url_decode(String(x_b64), client_pub_key, 66);
  int y_len = base64_url_decode(String(y_b64), client_pub_key + 66, 66);

  if (x_len != 66 || y_len != 66)
  {
  ...
  respDoc["kty"] = "EC";
  respDoc["crv"] = "P-521";
  respDoc["x"] = base64_url_encode(shared_point, 66);
  respDoc["y"] = base64_url_encode(shared_point + 66, 66);
...
void handlePub()
{
  ...
  key["kty"] = "EC";
  key["crv"] = "P-521";
  key["x"] = base64_url_encode(admin_public_key, 66);
  key["y"] = base64_url_encode(admin_public_key + 66, 66);
  key["alg"] = "ECDH-ES";
...
void handleActivate()
{
  ...
  uint8_t eph_pub_key[132];
  base64_url_decode(epk["x"].as<String>(), eph_pub_key, 66);
  base64_url_decode(epk["y"].as<String>(), eph_pub_key + 66, 66);
  ...
  memcpy(tang_sig_private_key, encrypted_tang_sig_key, 66);
  ...
  memcpy(tang_exc_private_key, encrypted_tang_exc_key, 66);
