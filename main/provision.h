...
static const uint8_t IO_KEY[32] = {
    0x2c, 0x43, 0x34, 0x9c, 0x4c, 0xe6, 0x70, 0xf3,
    0xcb, 0x10, 0xef, 0xcc, 0x56, 0xf0, 0xd0, 0xc4,
    0x03, 0x2c, 0x45, 0x9f, 0xf3, 0xcb, 0x29, 0x27,
    0x22, 0x8e, 0x93, 0x3c, 0xfe, 0x6e, 0x87, 0xed};
...
static const uint8_t ATECC_SLOT_CONFIG[32] = {
    0x81, 0x00, 0x81, 0x20, 0x81, 0x20, 0x81, 0x20,
    0x84, 0x20, 0x84, 0x20, 0xc6, 0x46, 0x00, 0x00,
    0x00, 0x00, 0x00, 0x20, 0x9a, 0x4a, 0x84, 0x20,
    0x84, 0x20, 0x9a, 0x4a, 0x00, 0x00, 0x00, 0x00};

// KeyConfig (Bytes 96-127): 32 bytes for 16 slots (2 bytes each)
static const uint8_t ATECC_KEY_CONFIG[32] = {
    0x33, 0x00, 0x13, 0x00, 0xd3, 0x09, 0x93, 0x0a,
    0x53, 0x00, 0x53, 0x10, 0x7c, 0x00, 0x00, 0x00,
    0x00, 0x00, 0x10, 0x00, 0x5c, 0x00, 0x53, 0x00,
    0x53, 0x00, 0x1c, 0x00, 0x00, 0x00, 0x00, 0x00};
...
{
  ESP_LOGI(TAG_PROVISION, "=== Starting ATECC608B Configuration Provisioning ===");
  ...
  ESP_LOGI(TAG_PROVISION, "ATECC608B configuration provisioned and locked successfully!");
  ...
  {
    ESP_LOGE(TAG_PROVISION, "Verification failed: Config zone still appears unlocked");
    return false;
  }
}
...
bool provision_atecc608b_data_zone()
{
  ...
  {
    ESP_LOGW(TAG_PROVISION, "Data zone is already locked! Cannot provision keys.");
    return false;
  }

  if (!is_atecc608b_config_locked())
  {
    ESP_LOGE(TAG_PROVISION, "Config zone MUST be locked before generating keys (GenKey).");
    return false;
  }
  ...
  // Note: The 0x04 prefix byte is stripped. The ATECC608B expects exactly 64 bytes.
  static const uint8_t SLOT9_PUB_KEY[64] = {
      0x76, 0xc1, 0xa2, 0xe9, 0x63, 0xda, 0x58, 0x41, 0x12, 0x4e, 0xe7, 0xc5, 0x3b, 0xeb, 0x2d, 0xad,
      0x72, 0xf4, 0xc4, 0x61, 0xb8, 0x4a, 0x65, 0xb7, 0xc7, 0x91, 0xdd, 0x59, 0xf9, 0x0a, 0xad, 0xf0,
      0x6f, 0x13, 0xf2, 0xb6, 0x29, 0x05, 0x4f, 0xab, 0x98, 0xdc, 0xfb, 0x93, 0xab, 0xbd, 0x90, 0xd1,
      0xea, 0x92, 0x91, 0x0a, 0xfe, 0x95, 0x7c, 0xf6, 0xc7, 0x97, 0x41, 0x8e, 0x96, 0x6c, 0xaa, 0x15};
  ...
  for (int i = 0; i < sizeof(genkey_slots) / sizeof(genkey_slots[0]); i++)
  {
    ...
    ESP_LOGI(TAG_PROVISION, "Successfully generated Private Key in Slot %d.", slot);
    ...
    printf("--> Copy this public key for Slot %d: 04", slot); // Prepending the 0x04 uncompressed prefix
    for (int j = 0; j < 64; j++)
    {
      printf("%02x", pubkey[j]);
    }
  ...
  ESP_LOGI(TAG_PROVISION, "ATECC608B Data Zone provisioned and locked successfully!");
  return true;
}
