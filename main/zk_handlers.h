...
static esp_err_t handle_zk_root(httpd_req_t *req) {
  ...
  httpd_resp_set_hdr(
      ...
      "default-src 'none'; "
      "script-src 'sha256-kEixpl88qlpp+XVsW7rm5Ajaaq/YUvviS1MtQBSGtqA='; "
      "style-src 'sha256-xxSFVVC2ZdqsOT4eVrW+anOuPfexGhhIkwxTKcE6vTA=' "
...
static esp_err_t handle_zk_status(httpd_req_t *req) {
  ...
  int len = snprintf(
      ...
      keystore.has_exchange_key() ? "true" : "false", keystore.gen, uptime_ms,
      (unsigned)zk_auth.get_failed_attempts(), (unsigned)retry_after);
  if (have_rssi) {
    len += snprintf(response + len, sizeof(response) - len, ",\"wifi_rssi\":%d",
                    ap_info.rssi);
  }
