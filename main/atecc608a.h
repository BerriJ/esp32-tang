...
void atecc608B_print_config()
{
  ...
  ESP_LOGI(TAG_ATECC, "Key Configuration Details:");
  ESP_LOGI(TAG_ATECC, "---------------------------");
  ...
  ESP_LOGI(TAG_ATECC, "Revision: %02X %02X %02X %02X",
           config_data[4], config_data[5], config_data[6], config_data[7]);

  // I2C Address (byte 16)
  ESP_LOGI(TAG_ATECC, "I2C Address: 0x%02X", config_data[16]);

  // OTP Mode (byte 18)
  ESP_LOGI(TAG_ATECC, "OTP Mode: 0x%02X", config_data[18]);

  // Chip Mode (byte 19)
  printf("Chip Mode: 0x%02X ", config_data[19]);
  if (config_data[19] & 0x01)
    printf("[I2C_Address_UserExtraAdd] ");
  ...
  for (int slot = 0; slot < 16; slot++)
  {
    ...
    printf("  Slot %2d: SlotConfig=0x%04X, KeyConfig=0x%04X", slot, slot_config, key_config);
  ...
  // Lock status
  ESP_LOGI(TAG_ATECC, "\nLock Status:");
  ESP_LOGI(TAG_ATECC, "  Config Zone Lock: 0x%02X %s", config_data[87],
           config_data[87] == 0x00 ? "[LOCKED]" : "[UNLOCKED]");
  ESP_LOGI(TAG_ATECC, "  Data/OTP Zone Lock: 0x%02X %s", config_data[86],
