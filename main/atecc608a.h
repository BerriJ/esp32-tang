...
bool atecc608B_init()
{
  ...
  ESP_LOGI(TAG_ATECC, "Configuring ATECC608B: SDA=GPIO%d, SCL=GPIO%d, Address=0x%02X (7-bit: 0x%02X)",
...
bool atecc608B_read_config()
{
  ...
  if (status != ATCA_SUCCESS)
  {
    ESP_LOGE(TAG_ATECC, "ERROR: Failed to read config zone, status 0x%02X", status);
    ...
    {
      status = atcab_read_zone(ATCA_ZONE_CONFIG, 0, block, 0, &g_atecc_config_data[block * 4], 4);
      if (status != ATCA_SUCCESS)
      {
        ESP_LOGE(TAG_ATECC, "ERROR: Failed to read block %d, status 0x%02X", block, status);
        read_success = false;
        break;
      }
    }
...
void atecc608B_print_config()
{
  ...
  if (status != ATCA_SUCCESS)
  {
    ESP_LOGE(TAG_ATECC, "ERROR: Failed to read serial number, status 0x%02X", status);
  ...
  printf("[Byte 16] I2C_Address: 0x%02X (7-bit: 0x%02X)\n",
         config_data[16], config_data[16] >> 1);
  ...
  for (int slot = 0; slot < 16; slot++)
  {
    ...
    printf("  Slot %2d [Bytes %2d-%2d]: 0x%02X 0x%02X (",
           slot, offset, offset + 1, slot_config_low, slot_config_high);
  ...
  for (int slot = 0; slot < 16; slot++)
  {
    ...
    printf("  Slot %2d [Bytes %2d-%2d]: 0x%02X 0x%02X (",
           slot, offset, offset + 1, key_config_low, key_config_high);
...
/**
 * Get ATECC608B configuration as JSON string (caller must free the returned string)
 */
char *atecc608B_get_config_json()
{
  if (!g_atecc_config_valid)
  {
    ESP_LOGE(TAG_ATECC, "Config data not valid. Call atecc608B_print_config() first.");
    return NULL;
  }
  ...
  sprintf(revision_str, "%02X%02X%02X%02X",
  ...
  for (int slot = 0; slot < 16; slot++)
  {
    ...
    cJSON_AddItemToArray(slot_config_arr, cJSON_CreateString(slot_config_low_str));
    cJSON_AddItemToArray(slot_config_arr, cJSON_CreateString(slot_config_high_str));
    ...
    cJSON_AddItemToArray(key_config_arr, cJSON_CreateString(key_config_low_str));
    cJSON_AddItemToArray(key_config_arr, cJSON_CreateString(key_config_high_str));
    ...
    cJSON_AddBoolToObject(slot_obj, "encrypt_read", (slot_config & 0x4000) != 0);
