...
static esp_err_t handle_activate(httpd_req_t *req)
{
  if (is_active)
  {
    httpd_resp_send_err(req, HTTPD_400_BAD_REQUEST, "Already active");
    return ESP_FAIL;
  }
  ...
  ESP_LOGI(TAG_HANDLERS, "Activation request (decryption may take 20-30s)...");
  ...
  {
    cJSON_Delete(req_doc);
    ESP_LOGE(TAG_HANDLERS, "Decryption failed - Nuking configuration...");
    keystore.nuke();
    httpd_resp_send_err(req, HTTPD_401_UNAUTHORIZED, "Decryption failed - nuked configuration...");
    return ESP_FAIL;
  }
  ...
  {
    ESP_LOGE(TAG_HANDLERS, "Invalid password - Nuking configuration...");
    keystore.nuke();
    httpd_resp_sendstr_chunk(req, "Wrong password, nuked configuration.");
  ...
  is_active = true;
  ESP_LOGI(TAG_HANDLERS, "Server ACTIVATED");
  httpd_resp_sendstr(req, "Server activated");
  return ESP_OK;
}
